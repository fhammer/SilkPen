...
class RibbonStyle implements Style {
...

    {
        ...
        mColors = ColorRender.gradientColor("#ffffff", "#ffffff", MAX_SIZE);
    }
    ...
    public void stroke(Canvas c, float x, float y) {
        ...
        this.y = y;
        if (!isRecording)
            mNodes.add(new Node(x, y, Node.ACTION_MPVE));
    }
    ...
    public void strokeStart(float x, float y) {
        ...
        if (!isRecording)
            mNodes.add(new Node(x, y, Node.ACTION_DOWN));
    }

    @Override
    public void setColor(int color) {
        Log.d("Color_", getColorString(color));

        mColors = ColorRender.gradientColor("#" + getColorString(paint.getColor()), "#" + getColorString(color), MAX_SIZE);
    ...
    @Override
    public void startPlayRecode(final Canvas c) {
        new Thread() {
            @Override
            public void run() {
                isRecording = true;
                for (Node n : mNodes) {
                    if (n.getType() == Node.ACTION_DOWN)
                        strokeStart(n.getX(), n.getY());
                    else
                        stroke(c, n.getX(), n.getY());
